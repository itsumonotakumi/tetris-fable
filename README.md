...
**ひとり / 対COM** では、下の2つのキー配置が**同時に有効**です。手の好みに合わせてどちらでも操作できます。

| 操作 | 左手モード（WASD） | 右手モード（矢印キー） |
|---|---|---|
| 左右移動 | A / D | ← / → |
| ソフトドロップ | S | ↓ |
| ハードドロップ | W | ↑ / Space |
| 右回転 | G | X |
| 左回転 | F | Z |
| ホールド | 左Shift / Q | C / 左Shift |

> **右手モード**は「右手で移動（矢印キー）・左手で操作（Z/X/C）」のレイアウトです。矢印キーでそのまま遊べます。

### 2P キーボード（ふたりで対戦）

ふたりで対戦するときは1台のキーボードを左右で分け合うため、**1P＝左手モード（WASD）固定 / 2P＝矢印キー**になります。
...
ゲームパッドは **1台目が1P、2台目が2P** に自動割り当てされます（標準マッピング対応パッド）。メニューもキーボード・パッドどちらでも操作できます。ポーズはキーボードの Esc / P でも可能です。
...
node test/keys.js       # PC 2キー配置（矢印/WASD）と対人戦の分担
```
